...
const wordBank = {
  ...
  "家具": { high: ["木桌", "化妝台", "沙發", "椅子", "衣櫥"], low: ["冷氣", "洗衣機", "鞋櫃", "馬桶", "冰箱"] },
  ...
  "飲料": { high: ["紅茶", "麥茶", "多多綠", "牛奶", "咖啡"], low: ["普洱茶", "抹茶", "燕麥奶", "香檳", "茉莉花茶"] }
};
...
// 💡 獨立出 Firebase 上傳模組，供「正常結束」與「切換APP自動救援」共用
async function uploadDataToFirebase(feedbackObj, isEmergency = false) {
  ...
  try {
    await setDoc(doc(db, "results", sub_id), { 
      ...
      saveType: isEmergency ? 'Emergency_AutoSave' : 'Normal_Submit' 
    });
  } catch(e) {
            ...
            if (document.visibilityState === 'hidden') {
                ...
                if (form) {
                    ...
                    formData.forEach((val, key) => { 
                        currentFeedback[key] = (val === 'on') ? true : val; 
                    });
                }
                uploadDataToFirebase(currentFeedback, true); 
            }
        };
